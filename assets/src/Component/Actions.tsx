...
export function Actions(props: Props) {
  const {
    filesize, editBtn, deleteBtn, setToDelete, renderData, filename, url, modifyId,
  } = props;
  ...
  return (
    <div className="hovered-nav">
        ...
        /* eslint jsx-a11y/label-has-associated-control: ["error", { assert: "either" } ] */
          <label title="Delete" htmlFor={renderData?.deleteId}>
            <button
              ...
              type="button"
              onClick={() => { if (undefined !== setToDelete) { setToDelete(true); } }}
              onKeyDown={() => { if (undefined !== setToDelete) { setToDelete(true); } }}
            >
              <span className="btn-label">
      ...
      <div>
        {(undefined !== filesize) && (
          <span className="text-right file-size">{filesize}</span>
        )}
        {(undefined !== url) && (
          <span>
            <a href={url}>
